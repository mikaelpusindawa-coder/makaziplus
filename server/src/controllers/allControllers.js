...
exports.initiatePayment = async (req, res) => {
  try {
    ...
    const idempotencyKey = `${plan}-${Date.now()}-${Math.random().toString(36).substr(2, 8)}`;

    const [r] = await db.execute(
      'INSERT INTO payments (user_id, property_id, amount, plan, method, phone, transaction_id, idempotency_key, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())',
      [req.user.id, safePropId, safeAmount, plan, method, safePhone, txnId, idempotencyKey, 'pending']
    );
    ...
    setTimeout(async () => {
      try {
        ...
        if (plan === 'pro' || plan === 'owner') {
          ...
          await db.execute(
            'INSERT INTO subscriptions (user_id, plan, start_date, end_date, status) VALUES (?, ?, CURDATE(), ?, ?)',
            [req.user.id, plan, endDate.toISOString().split('T')[0], 'active']
          );
        }
        if (plan === 'boost' && safePropId) {
          await db.execute('UPDATE properties SET is_premium = 1 WHERE id = ? AND owner_id = ?', [safePropId, req.user.id]);
        }
        console.log(`✅ Payment ${pid} completed successfully`);
      } catch (err) {
        console.error('Payment callback error:', err.message);
      }
    }, 3000);

    res.status(201).json({ success: true, transaction_id: txnId, payment_id: pid, message: 'Ombi la malipo limetumwa. Subiri uthibitisho kwenye simu yako.' });
  } catch (e) {
