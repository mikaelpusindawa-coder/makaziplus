...
if (!fs.existsSync(propertiesDir)) {
  ...
  console.log('✅ Created properties directory');
}
...
console.log(`📁 Properties images: ${propertiesDir}`);
...
const authLimiter = rateLimit({
  ...
  handler: async (req, res, next, opts) => {
    try {
      await db.execute(
        'INSERT INTO audit_log (action, ip_address, user_agent, status) VALUES (?, ?, ?, ?)',
        ['RATE_LIMIT_AUTH', req.ip, req.get('user-agent') || '', 'suspicious']
      );
    } catch {}
    ...
    try {
      ...
      const [result] = await db.execute(
        `INSERT INTO messages (from_user_id, to_user_id, property_id, message, is_read, created_at)
         VALUES (?, ?, ?, ?, 0, NOW())`,
        [uid, toId, safePropId, String(message).trim()]
      );
...
server.listen(PORT, () => {
  console.log(`🚀 MakaziPlus Server → http://localhost:${PORT}`);
  console.log(`📡 Socket.IO ready`);
  console.log(`🛡️ Security: Helmet + Rate Limiting + IP Block + Audit Log active`);
